...
def test_auto_mapper_fhir_patient(spark_session: SparkSession) -> None:
    ...
    mapper = AutoMapper(
        ...
        keys=["member_id"]).columns(patient=F.patient.map(
            id_=A.column("a.member_id"),
            identifier=FhirList(
                F.identifier.map(
                    use=FhirIdentifierUseCode.Usual,
                    value=A.column("a.member_id"),
                    type_=FhirCodeableConcept.map(coding=FhirCoding.map(
                        code=FhirIdentifierTypeCode.map("MR"))))),
            name=FhirList(
                F.human_name.map(use=FhirNameUseCode.map("usual"),
            ...
            maritalStatus=FhirCodeableConcept.map(
                coding=FhirCoding.map(code=F.codes.marital_status.Married,
    ...
    assert str(sql_expressions["patient"]) == str(
        struct(
            ...
            array(
                struct(
                    lit("usual").alias("use"),
                    struct(struct(lit("MR").alias("code")).alias(
                        "coding")).alias("type"),
            ...
            array(
                struct(
                    ...
                    array(lit("first_name"),
                          lit("middle_name")).alias("given"))).alias("name"),
            lit("female").alias("gender"),
            coalesce(to_date(col("date_of_birth"), 'yyyy-MM-dd'),
                     to_date(col("date_of_birth"), 'yyyyMMdd'),
                     to_date(col("date_of_birth"),
                             'MM/dd/yy')).alias("birthDate"),
            struct(
                struct(
                    lit("http://terminology.hl7.org/CodeSystem/v3-MaritalStatus"
                        ).alias("system"),
                    lit("M").alias("code"),
                ).alias("coding")).alias("maritalStatus")).alias("patient"))
    ...
    assert result_df.where("member_id == 1").selectExpr(
        "patient.name[0].use").collect()[0][0] == "usual"
    assert result_df.where("member_id == 1").selectExpr(
        "patient.name[0].family").collect()[0][0] == "Qureshi"
