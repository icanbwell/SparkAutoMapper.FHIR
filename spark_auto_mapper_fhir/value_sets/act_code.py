...
class ActCode(GenericTypeCode):
    """
    v3.ActCode
    From: http://terminology.hl7.org/ValueSet/v3-ActCode in v3-codesystems.xml
    ...
    encounter, charge financial transaction, etc.) is specified with a code from
    one of several, typically external, coding systems.  The coding system will
    depend on the class of Act, such as LOINC for observations, etc. Conceptually,
    the Act.code must be a specialization of the Act.classCode. This is why the
    structure of ActClass domain should be reflected in the superstructure of the
    ActCode domain and then individual codes or externally referenced vocabularies
    subordinated under these domains that reflect the ActClass structure.
    Act.classCode and Act.code are not modifiers of each other but the Act.code
    concept should really imply the Act.classCode concept. For a negative example,
    ...
    potassium". This mutually modifying use of Act.code and Act.classCode is not
    permitted.
    """
    ...
    """
    codeset: FhirUri = "http://terminology.hl7.org/ValueSet/v3-ActCode"
    """
...
class ActCodeValues:
    ...
    From: http://terminology.hl7.org/CodeSystem/v3-ActCode in v3-codesystems.xml
    """
    _workersCompensationProgram = ActCode("WRKCOMP")
    """
